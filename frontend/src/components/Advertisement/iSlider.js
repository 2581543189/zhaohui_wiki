...
iSlider.prototype = {
  ...
  init: function() {
    ...
    this._sessionKey = btoa(
      encodeURIComponent(this._sessionKey + this.wrap.id + this.wrap.className)
    );

    var lastLocateIndex = parseInt(sessionStorage[this._sessionKey], 10);
    this.index =
    ...
    this.wrap.style.cssText +=
      ...
      (this.opts.fullScr ? "width:100%;height:100%" : "");
  ...
  _touchstart_mouse: function(e) {
    var self = this;
    //if(e.touches.length !== 1){return;}//如果大于1个手指，则不处理

    this.lockSlide = false;
    this._touchstartX = e.pageX;
    this._touchstartY = e.pageY;

    this.touchInitPos = this.opts.isVertical ? e.pageY : e.pageX; // 每次move的触点位置
    this.deltaX1 = this.touchInitPos; //touchstart的时候的原始位置
  ...
  _touchmove_mouse: function(e) {
    ...
    var gx = Math.abs(e.pageX - this._touchstartX);
    var gy = Math.abs(e.pageY - this._touchstartY);
    ...
    if (this.opts.noslide && this.opts.noslide.indexOf(this.index) >= 0) {
      ...
      if (
        !this.opts.noslideBack ||
        (e.pageY - this._touchstartY < 0 || e.pageX - this._touchstartX < 0)
      ) {
        return;
      }
    }

    var currentX = this.opts.isVertical ? e.pageY : e.pageX;
    this.deltaX2 = currentX - this.deltaX1; //记录当次移动的偏移量
    this.totalDist = this.startPos + currentX - this.touchInitPos;
  ...
  _loading: function() {
    ...
    for (var i = 0; i < imgurls.length; i++) {
      ...
      imgs[i].onload = imgs[i].onerror = imgs[i].onabort = _fun;
    }
