import { queryLeetcode ,deleteLeetcode,addLeetcode ,updateLeetcode,distinctValue} from '@/services/data';
...
export default {
    ...
    effects: {
        ...
        *add({ payload,callback }, { call, put,select }) {
            const response = yield call(addLeetcode, payload);
            if(response.error!=1){
                yield put({
        ...
        *delete ({ payload }, { call, put ,select }){
            const response = yield call(deleteLeetcode, payload);
            if(response.error!=1){
        ...
        *updateStep2({ payload }, { call, put ,select }){
            const response = yield call(updateLeetcode, payload);
            if(response.error!=1){
                yield put({
                    type: 'setUpdateModalVisible',
                    payload:{
                ...
                openNotification('success','更新题目['+response.data.id+']成功')
            }else{
                openNotification('error',response.message);
            }
