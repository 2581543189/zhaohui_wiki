import { queryLeetcodeExp ,deleteLeetcodeExp,addLeetcodeExp ,distinctValue,queryLeetcode} from '@/services/data';
...
export default {
    ...
    effects: {
        ...
        *fetch({ payload }, { call, put,select }) {
            ...
            const formValues = yield select(state => state.data_leetcode_exp.formValues);
            const param = {
                ...formValues,
                ...payload,
            }
            param['type'] = 'LEETCODE_EXP'
            const response = yield call(queryLeetcodeExp, param);
            if(response.error!=1){
                yield put({
                    type: 'save',
                    payload: response,
        ...
        *add({ payload,callback }, { call, put,select }) {
            const response = yield call(addLeetcodeExp, payload);
            if(response.error!=1){
                yield put({
                    type: 'setModalVisible',
                    payload:false,
                });
                openNotification('success','新增题目['+response.data.name+']成功')
                const formValues = yield select(state => state.data_leetcode_exp.formValues);
                yield put({
                    type: 'fetch',
                    payload:{
        ...
        *delete ({ payload }, { call, put ,select }){
            const response = yield call(deleteLeetcodeExp, payload);
            if(response.error!=1){
                openNotification('success','删除题目['+response.data.id+']成功')
                const formValues = yield select(state => state.data_leetcode_exp.formValues);
                yield put({
                    type: 'fetch',
                    payload:{
        ...
        *getLeetcode({ payload }, { call, put,select }) {
        ...
        *getOneLeetcode({ payload }, { call, put,select }) {
            if(typeof(payload)=='undefined'||payload.name===null||payload.name===''){
                return;
            }
            const param ={
            ...
            if(response.error!=1){
                yield put({
                    type: 'setAddLeetcode',
                    payload: {
                        addLeetcode:response.data.list[0],
                    }
                });
            }else{
                openNotification('error',response.message);
    ...
    reducers: {
        ...
        setOptions(state, action){
            state.leetcodeOptions=action.payload.options;
            
            return{
                ...state,
            }
        },
        setAddLeetcode(state, action){
            return {
                ...state,
                addLeetcode: action.payload.addLeetcode,
            };
